...
def test_density():
    cell = Cell(volume_mcm3=4000.0, mass_ng=4.0)
    assert cell.density() == 0.001

    another_cell = Cell(volume_mcm3=30000.0, mass_ng=850.0)
    density = another_cell.density()
    assert density == 0.028 and round(density, 3)


def test_mitosis():
    parent_cell = Cell(volume_mcm3=8000.0, mass_ng=2.0)
    ...
    assert daughter_cell_b.mass_ng == daughter_cell_a.mass_ng
    assert daughter_cell_b.mass_ng == 1.0

    assert daughter_cell_a.density() == parent_density


def test_consume():
    predator_cell = Cell(volume_mcm3=500.0, mass_ng=1.0)
    prey_cell = Cell(volume_mcm3=100.0, mass_ng=0.3)
    predator_cell.consume(prey_cell)

    assert predator_cell.volume_mcm3 == 600.0
    assert predator_cell.mass_ng == 1.3

    assert prey_cell.volume_mcm3 == 0.0
    assert prey_cell.mass_ng == 0.0
